...
async def get_stats():
    ...
    try:
        # Total blocked domains in the blocklist
...
async def get_top_blocked_domains(limit: int = 10):
    """Return top blocked domains by query count"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
async def get_blocked_logs(limit: int = 100):
    """Return recent blocked query logs"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
async def get_allowed_logs(limit: int = 100):
    """Return recent allowed query logs"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
async def get_all_domains(limit: int = 1000):
    """Return list of all domains in the blocklist"""
    conn = get_db_connection()
    cursor = conn.cursor()
